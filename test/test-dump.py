...
class TestCreateBid(unittest.TestCase):
    ...
    def test_bid_exists_after_create(self, postcode):
        bid_id = dump.create_bid(postcode)
        self.assertEqual(bid_id, dump.get_bid(bid_id).code)
    ...
    def test_bid_has_default_values(self):
        bid_id = dump.create_bid(10179)
        created_bid = dump.get_bid(bid_id)
    ...
    def test_bid_has_custom_values(self, time, date):
        bid_id = dump.create_bid(76137, time=time, date=date)
        created_bid = dump.get_bid(bid_id)
